...
pub fn day3() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let vec_string:Vec<String> = re.find_iter(file_string.as_str()).map(|mat| (mat.as_str().to_string())).collect();
    ...
    let digits_str = re.find(mul.as_str())
    .map(|mat| mat.as_str().to_string()).unwrap();
